...
import { formatCommand, runCommand, workspaceRoot } from "../../shared/paths/workspace.js";
...
export function doctorView(): React.ReactElement {
  ...
  return createSection("Doctor", [
    ...
    createLine(`Default schedule: every ${DEFAULT_APP_CONFIG.schedule.intervalMinutes} minutes`, "cyan"),
...
export async function installView(): Promise<React.ReactElement> {
  ...
  return createSection("Install", [
    ...
    createLine("Mobile install is handled by Pubwave CLI setup or `techbrief mobile install`.", "cyan"),
    createLine(
      rootInstall.ok ? "Install flow completed for workspace dependencies." : "Install flow completed with warnings.",
      rootInstall.ok ? "cyan" : "yellow"
    )
  ]);
}
...
export async function buildWorkspaceView(): Promise<React.ReactElement> {
  ...
  return createSection("Build", [
    ...
    createLine("Mobile build/run is handled by Pubwave CLI mobile commands.", "cyan"),
    createLine(
      webBuild.ok ? "Web bundle completed." : "Build flow completed with warnings.",
      webBuild.ok ? "cyan" : "yellow"
    )
  ]);
}
